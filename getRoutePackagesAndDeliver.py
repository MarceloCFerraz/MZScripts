...
if __name__ == "__main__":
    ...
    if events is not None:
        ...
        if not no_itinerary_generated(
            ...
            # now sync back data into the response

            for itinerary in itinerary_ids:
                response["itineraries"].append(
                    ITINERARIES[itinerary].get("pkgsAndStops")
                )
            ...
            # lastly, mark packages as delivered

            print("Marking all packages delivered with Thread Pool")
            with ThreadPoolExecutor() as pool:
                for package in response["uniquePackages"]:
                    pool.submit(packages.mark_package_as_delivered, env, orgId, package)
