...
def checkIfNeedsRepetition(emailList):
    ...
    other_providers = []
    common_providers = ["@milezero.com", "@capstonelogistics.com", "@lean-tech.io"]
    common_providers_count = 0

    for email in emailList:
        email_provider = re.search("@[a-zA-Z0-9_.-]+.[a-zA-Z]{2,}", email)

        if email_provider is not None:
            email_provider = email_provider.group()
...
def start(channels_dict):
    ...
    common.cprint(
        "More > Slack Connect > View Invitations", "light_green", attrs=["bold"]
    )
