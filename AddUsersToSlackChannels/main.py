from .utils import automation, common, files, get, gui


def main():
    accounts_file = files.get()
    sheet = accounts_file.active

    emails_index = get.emailsColumnIndex(sheet)
    channels_index = get.channelNameColumnIndex(sheet)

    if channels_index is not None and emails_index is not None:
        ...
        spaces = "========================"
        common.cprint(
            "\n\n{0} ATTENTION {0}\n".format(spaces), "light_red", attrs=["bold"]
        )
        print(
        ...
        for question_index in range(0, len(common.QUESTIONS)):
            if common.answerCheck(answer):
                ...
                print("----> Hit", end=" ")
                common.cprint("'enter'", "light_grey", attrs=["underline"], end=" ")
                print("if this step was completed")

                print("----> Type", end=" ")
                common.cprint("'n'", "light_grey", attrs=["underline"], end=" ")
        ...
        if common.answerCheck(answer):
            ...
            common.cprint(
                "Automation will start in {} seconds!".format(sleep),
                "light_yellow",
                attrs=["bold"],
            )
        ...
        common.cprint(
            "Your sheet doesn't have a 'Channel Name' or a 'User E-Mail' header\n"
...
if __name__ == "__main__":
    if files.exists():
        ...
        text = (
            common.colored("Channels file wasn't found.\n", "light_red", attrs=["bold"])
            + "Please, make sure to create a "
            + common.colored("new file ", attrs=["bold"])
            + "or "
            + common.colored("rename ", attrs=["bold"])
            + "the file you put the e-mails to "
            + common.colored("'accounts.xlsx'", "light_yellow", attrs=["underline"])
            + "!\n"
            + "Also make sure that it is "
            + common.colored("structurally equal ", attrs=["bold"])
            + "to "
            + common.colored("'model.xlsx'", "light_yellow", attrs=["underline"])
            + " present in this project folder."
        )
        print(text)
